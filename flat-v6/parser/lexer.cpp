...
Token Lexer::advance()
{
	...
	if (isDigit(input[position]))
	{
		...
		if (input[position] == '0' && (position + 1) < input.length() && input[position + 1] == 'b')
		{
			position += 2;
		...
		if (position < input.length() && (input[position] == 'i' || input[position] == 'u'))
		{
			position++;
			while (position < input.length() && isDigit(input[position]))
				position++;
	...
	if (input[position] == '\'')
	{
		...
		size_t start = position;
		if (input[position] == '\'')
			error(position, "Empty char literal");

		advanceChar();

		if (!(position < input.length() && input[position] == '\''))
			error(position, "Unterminated char literal");
	...
	if (position < input.length() && input[position] == '\\')
	{
		...
		if (position < input.length() && isDigit(input[position])) // octal char literal
		{
			...
			if ((position - numStart) > 3)
				error(numStart, "Too many digits for octal char literal");
		...
		else if (position < input.length() && input[position] == 'x') // hex char literal
		{
			...
			if ((position - numStart) == 0)
				error(numStart, "Hex char literal cannot have zero digits");
		...
		else if (position < input.length() && input[position] == 'u') // 0xhhhh unicode code point
		{
			...
			if ((position - numStart) != 4)
				error(numStart, "2 byte Unicode code point (\\u) must have 4 digits");
		...
		else if (position < input.length() && input[position] == 'U') // 0xhhhhhhhh unicode code point
		{
			...
			if ((position - numStart) != 8)
				error(numStart, "4 byte Unicode code point (\\U) must have 8 digits");
		...
		{
			error(position, "Unexpected EOF in escape sequence");
			return "";
		}
	}
	else if (position < input.length())
...
bool Lexer::expect(Token expected)
{
	...
	position = before;
	error(position, "Unexpected Token " + std::string(value) + ", expected " + TokenNames[(size_t)expected]);
	return false;
}