...
{
public:
    VoidType(TypeContext& ctx) : Type(ctx) {}
...
{
public:
    BoolType(TypeContext& ctx) : Type(ctx) {}
...
public:
    CharType(TypeContext& ctx, size_t bitSize) : Type(ctx), bitSize(bitSize) {}
...
{
public:
    StringType(TypeContext& ctx) : Type(ctx) {}
...
public:
    StructType(
        ...
        std::vector<std::pair<std::string, Type*>> const& members = {})
...
public:
    ...
    void addField(std::string const& fieldName, Type* fieldType)
    {
        return fields.push_back(std::pair(fieldName, fieldType));
    }
};
...
public:
    PointerType(Type* base) : Type(base->ctx), base(base) {}
...
public:
    ArrayType(Type* base) : Type(base->ctx), base(base) {}
